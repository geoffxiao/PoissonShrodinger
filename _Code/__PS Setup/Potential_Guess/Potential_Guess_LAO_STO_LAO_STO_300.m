...
FILE_NAME = 'LAO_STO5nm_LAO_STO'; % Save to
...
x = [0 : h : 24e-9]'; % Global variable for the x axis
interfaces = [190; 692; 881]; % index of the interfaces in the material
...
Eg3 = Eg1;
Eg4 = Eg2;
Eg_params = [Eg1, Eg2, Eg3, Eg4];

% Left material is above right material by this much
dEc12 = 2.5; % 1 is above 2 by this much
dEc_params = [dEc12 -dEc12 dEc12];

epsilon_r1 = 27; 
epsilon_r2 = 300; 
epsilon_r_params = [epsilon_r1, epsilon_r2, epsilon_r1, epsilon_r2];

% Electron and hole effective masses
M_es = [14; 14; 14; 14] * M;
M_hs = [1.2; 1.2; 1.2; 1.2] * M; 
...
LAO = LAO_Setup(5,1);
LAO = flipud(LAO);
Other_Charges = [LAO; zeros(500,1); LAO; zeros(numel(x) - numel(LAO) - interfaces(2) + 1,1)];

guess_init = [([1 : 191]' - 191) * -5 / (191); zeros(691,1); ([883 : 2401]' - 883) / (2401 - 883)];
