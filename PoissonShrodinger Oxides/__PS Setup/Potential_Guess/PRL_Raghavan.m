...
FILE_NAME = 'PRL_Raghavan'; % Save to
% 10 u.c. LAO on STO Substrate at 300K
...
x = [0 : h : 26e-9]'; % Global variable for the x axis
interfaces = [501; 602]; % index of the interfaces in the material
...
Eg1 = 5; % band gap in eV
Eg2 = 3.15; % band gap in eV
Eg3 = 3.22;
Eg_params = [Eg1, Eg2, Eg3];

% http://avs.scitation.org/doi/full/10.1116/1.4963833
% Left material is above right material by this much
dEc12 = 0.6;
dEc23 = 0.07; % 1 is above 2 by this much
...
Other_Charges = [zeros(interfaces(1),1); (3.4e18*1e9/(0.2)) * ones(20,1); zeros(2080,1)];
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Potential Guess %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
guess_potential = x * 2e-4 / (h);
calc_potential = guess_potential;
...
loop_counter_vec = [500 500 1000 Inf];
f_factor_vec = [0.02 0.03 0.01 0.01]; % the new input potential merging effect
...
N_adapt = [500 500 500 4000 5000 Inf];
N_adapt_vals = [300 300 500 300 400 250];
